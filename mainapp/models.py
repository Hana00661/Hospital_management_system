...
class Service(models.Model):
    ...
    name = models.CharField(max_length=255)         # name of clinical specialitis
    description = models.TextField(blank=True, null=True)       # description of clinical specialitis
    cost = models.DecimalField(max_digits=10, decimal_places=2)         # How mush the clinical specialitis cost
    available_doctors = models.ManyToManyField(doctor_models.Doctor, blank=True)        #every clinical specialitis have many doctors
...
class Billing(models.Model):
    ...
    status = models.CharField(max_length=120, choices=[('Paid', 'Paid'), ('Unpaid', 'Unpaid')])     # inline interation of choices
